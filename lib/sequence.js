    ...
    if (that._options.old_reference_fields) {
        that._counterModel.findOne({id : id, reference_value:  referenceValue}, function (err, counter) {
            ...
            if (counter) {
                ...
                const old_reference_id = `${id}_${ that._options.old_ref_field_suffix|| "old"}`;
                that._counterModel.findOne({id : old_reference_id, reference_value:  that._getCounterReferenceField(doc, true)}, function (err, counter) {
                    if (err) return callback(err);
                    if (!counter) return callback(new Error("Old Counter not found"));
                    that._counterModel.findOneAndUpdate(
                        { id: id, reference_value: that._getCounterReferenceField(doc, that._options.old_reference_fields) },
                        { seq: counter.seq + 1 } ,
                        { new: true, upsert: true, passRawResult: true },
                        function(err, counter) {
                            if (err) return callback(err);
